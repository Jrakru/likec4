...
import { _stage, _type, exact, isDynamicBranchCollection, isDynamicStep, isDynamicStepsSeries, isViewRuleAutoLayout, stepEdgeId, stepEdgePath, toLegacyParallel, } from '../../types';
...
class DynamicViewCompute {
    ...
    registerBranchStep(branchStack, id) {
    ...
    buildBranchTrail(branchStack) {
        return branchStack.map(entry => exact({
    ...
    buildStepId(rootIndex, branchStack) {
        if (!branchStack || branchStack.length === 0) {
    ...
    processLegacySteps(viewSteps) {
        const process = (step, stepNum, prefix) => {
        ...
        for (const step of viewSteps) {
            ...
            if (isDynamicBranchCollection(step)) {
                const walkBranchEntries = (entries, prefix) => {
    ...
    processBranchAwareSteps(viewSteps) {
        const branchStack = [];
        const emitInBranch = (step, rootIndex) => {
            const id = this.buildStepId(rootIndex, branchStack);
            this.emitStep(step, id, branchStack);
        };
        const emitAtRoot = (step, rootIndex) => {
            const id = this.buildStepId(rootIndex);
            this.emitStep(step, id);
        };
        const processLegacyParallelAtRoot = (parallel, rootIndex) => {
            let nestedIndex = 1;
            for (const entry of parallel?.__parallel ?? []) {
                ...
                const id = stepEdgeId(rootIndex, nestedIndex);
                this.emitStep(entry, id);
                nestedIndex++;
            }
        };
        const processBranchEntries = (entries, rootIndex) => {
            for (const entry of entries) {
                const legacyNested = toLegacyParallel(entry);
                if (legacyNested) {
                    for (const nestedEntry of legacyNested.__parallel ?? []) {
                        if (isDynamicStepsSeries(nestedEntry)) {
                            ...
                            emitInBranch(nestedEntry, rootIndex);
                        }
                    }
                    continue;
                }
                ...
                if (isDynamicBranchCollection(entry)) {
                    const legacy = toLegacyParallel(entry);
                    if (legacy) {
        ...
        const processBranchCollection = (branch, rootIndex) => {
