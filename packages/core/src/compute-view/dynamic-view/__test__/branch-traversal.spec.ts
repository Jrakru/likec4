...
describe('DynamicViewCompute branch traversal', () => {
  ...
  it('should generate correct IDs for series/chains in nested branches', () => {
          ...
          [
            {
              seriesId: '/series@1',
              __series: [
                $step('support -> cloud.frontend.dashboard'),
                $step('cloud.frontend.dashboard -> cloud.backend.storage'),
              ],
            } as any,
          ],
          {
            pathName: 'anotherChain',
          },
        ),
      ] as any,
    }
    ...
    // Verify correct number of edges (3 from first path + 2 from second path)
    expect(computed.edges).toHaveLength(5)

    // Verify step IDs are correctly generated with dot notation
    expect(computed.edges.map(edge => edge.id)).toEqual([
      'step-01.01.01',
      'step-01.01.02',
      'step-01.01.03',
    ...
    // Verify branch trail is consistent for series steps
    expect(computed.edges[0]?.branchTrail?.[0]?.indexWithinPath).toBe(1)
    expect(computed.edges[1]?.branchTrail?.[0]?.indexWithinPath).toBe(2)
    expect(computed.edges[2]?.branchTrail?.[0]?.indexWithinPath).toBe(3)
    expect(computed.edges[3]?.branchTrail?.[0]?.indexWithinPath).toBe(1)
    expect(computed.edges[4]?.branchTrail?.[0]?.indexWithinPath).toBe(2)

    // Verify all edges in first path have correct metadata
    for (let i = 0; i < 3; i++) {
      expect(computed.edges[i]?.branchTrail?.[0]?.pathId).toBe('/parallel@0/path@0')
      expect(computed.edges[i]?.branchTrail?.[0]?.pathIndex).toBe(1)
    }

    // Verify all edges in second path have correct metadata
    for (let i = 3; i < 5; i++) {
      expect(computed.edges[i]?.branchTrail?.[0]?.pathId).toBe('/parallel@0/path@1')
      expect(computed.edges[i]?.branchTrail?.[0]?.pathIndex).toBe(2)
    }

    // Verify branchCollections includes correct edgeIds for each path
    expect(computed.branchCollections).toBeDefined()
    expect(computed.branchCollections![0]?.paths[0]?.edgeIds).toEqual([
      'step-01.01.01',
      'step-01.01.02',
      'step-01.01.03',
    ])
    expect(computed.branchCollections![0]?.paths[1]?.edgeIds).toEqual([
      'step-01.02.01',
      'step-01.02.02',
    ])
  })
