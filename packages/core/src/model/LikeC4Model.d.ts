...
import type { Any, Aux, AuxFromDump, ComputedLikeC4ModelData, IteratorLike, LayoutedLikeC4ModelData, LikeC4ModelDump, LikeC4Project, ModelGlobals, ParsedLikeC4ModelData, Specification, WhereOperator } from '../types';
...
export declare class LikeC4Model<A extends Any = aux.Unknown> {
    /**
     ...
     */
    isParsed(this: LikeC4Model<any>): this is LikeC4Model<aux.toParsed<A>>;
    /**
     * Type guard the model to the layouted stage.
     */
    isLayouted(this: LikeC4Model<any>): this is LikeC4Model<aux.toLayouted<A>>;
    /**
     * Type guard the model to the computed stage.
     */
    isComputed(this: LikeC4Model<any>): this is LikeC4Model<aux.toComputed<A>>;
    /**
     ...
     */
    findView(viewId: aux.LooseViewId<A>): LikeC4ViewModel<A, $View<A>> | null;
    /**
...
export declare namespace LikeC4Model {
    const EMPTY: LikeC4Model<Aux<"parsed", E, D, V, PID, Spec>>;
    type Parsed<A = aux.Unknown> = aux.Unknown extends A ? LikeC4Model<aux.UnknownParsed> : A extends Aux<any, infer E, infer D, infer V, infer PID, infer Spec> ? LikeC4Model<Aux<'parsed', E, D, V, PID, Spec>> : never;
    type Computed<A = aux.Unknown> = aux.Unknown extends A ? LikeC4Model<aux.UnknownComputed> : A extends Aux<any, infer E, infer D, infer V, infer PID, infer Spec> ? LikeC4Model<Aux<'computed', E, D, V, PID, Spec>> : never;
    type Layouted<A = aux.Unknown> = aux.Unknown extends A ? LikeC4Model<aux.UnknownLayouted> : A extends Aux<any, infer E, infer D, infer V, infer PID, infer Spec> ? LikeC4Model<Aux<'layouted', E, D, V, PID, Spec>> : never;
    type Node<A = aux.Unknown> = A extends aux.AnyAux ? NodeModel<A> : never;
    type Element<A = aux.Unknown> = A extends aux.AnyAux ? ElementModel<A> : never;
    type Relationship<A = aux.Unknown> = A extends aux.AnyAux ? RelationshipModel<A> : never;
    type View<A = aux.Unknown> = A extends aux.AnyAux ? $ViewModel<A> : never;
    type DeploymentNode<A = aux.Unknown> = A extends aux.AnyAux ? DeploymentNodeModel<A> : never;
    type DeployedInstance<A = aux.Unknown> = A extends aux.AnyAux ? DeployedInstanceModel<A> : never;
    type AnyRelation<M = aux.Unknown> = M extends aux.AnyAux ? RelationshipModel<M> | DeploymentRelationModel<M> : never;
}
//# sourceMappingURL=LikeC4Model.d.ts.map