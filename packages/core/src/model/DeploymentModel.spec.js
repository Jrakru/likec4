...
describe('LikeC4DeploymentModel', () => {
    const model = Builder
        ...
        .deployment(({ nd, vm, instanceOf }, d) => d(nd('customer').with(instanceOf('customer')), nd('prod'), nd('prod.z1').with(vm('vm1').with(instanceOf('ui', 'cloud.ui', {
        title: 'Prod/Zone 1/UI',
    })), vm('vm2', {
        tags: ['tag2'],
    }).with(instanceOf('backend-with-tags', 'cloud.backend', {
        tags: ['tag3'],
    }))), nd('prod.z2').with(vm('vm1').with(instanceOf('cloud.ui')), vm('vm2').with(instanceOf('cloud.backend'))), nd('prod.infra').with(instanceOf('infra.db'))))
