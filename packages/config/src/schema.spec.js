...
describe('ProjectConfig schema', () => {
    describe('validateProjectConfig', () => {
        ...
        describe('imageAliases field', () => {
            ...
            it('should reject keys not starting with @', ({ expect }) => {
                ...
                expect(() => validateConfig(config)).toThrow('Invalid image alias key(s): "icons" (must match /^@[A-Za-z0-9_-]*$/)');
            });
            it('should reject keys with invalid characters', ({ expect }) => {
                ...
                for (const key of invalidKeys) {
                    ...
                    expect(() => validateConfig(config), `Key "${key}" should be rejected`).toThrow('Invalid image alias key(s):');
                }
            });
    ...
    describe('validateImageAliases', () => {
        ...
        it('should reject keys not starting with @', ({ expect }) => {
            ...
            expect(() => validateImageAliases(invalidAliases)).toThrow('Invalid image alias key(s): "icons", "brand" (must match /^@[A-Za-z0-9_-]*$/)');
        });
        it('should reject keys with invalid characters', ({ expect }) => {
            const testCases = [
            ...
            for (const { key, expected } of testCases) {
                ...
                expect(() => validateImageAliases(aliases), `Key "${key}" should be rejected`).toThrow(`Invalid image alias key(s): ${expected} (must match /^@[A-Za-z0-9_-]*$/)`);
            }
        });
        it('should reject absolute paths as values', ({ expect }) => {
            const testCases = [
            ...
            for (const aliases of testCases) {
                ...
                if (entry) {
                    ...
                    expect(() => validateImageAliases(aliases), `Path "${value}" should be rejected`).toThrow('Invalid image alias value(s):');
                }
            }
        });
        it('should reject URLs as values', ({ expect }) => {
            ...
            for (const aliases of testCases) {
                ...
                if (entry) {
                    ...
                    expect(() => validateImageAliases(aliases), `URL "${value}" should be rejected`).toThrow(`Invalid image alias value(s): "${key} -> ${value}"`);
                }
            }
        });
        it('should report multiple invalid keys', ({ expect }) => {
            ...
            expect(() => validateImageAliases(aliases)).toThrow('Invalid image alias key(s): "icons", "brand", "@icons.old" (must match /^@[A-Za-z0-9_-]*$/)');
        });
        it('should report multiple invalid values', ({ expect }) => {
            ...
            expect(() => validateImageAliases(aliases)).toThrow('Invalid image alias value(s): "@icons -> /absolute/path", "@brand -> http://example.com"');
        });
