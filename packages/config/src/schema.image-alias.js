...
export const ImageAliasesSchema = z.record(z.string(), // PLAIN key schema - valibot JSON schema export-safe.
...
export function validateImageAliases(imageAliases) {
    ...
    if (invalidKeys.length || invalidValues.length) {
        ...
        if (invalidKeys.length) {
            parts.push(`Invalid image alias key(s): ${invalidKeys
                .map((k) => JSON.stringify(k))
                .join(', ')} (must match ${IMAGE_ALIAS_KEY_REGEX})`);
        }
        if (invalidValues.length) {
            parts.push(`Invalid image alias value(s): ${invalidValues
                .map((kv) => JSON.stringify(kv))
                .join(', ')} (must match ${IMAGE_ALIAS_VALUE_REGEX})`);
        }
