 ...
 */
function normalizeBranchKind(kind) {
    return kind === 'alternate' || kind === 'alt' ? 'alternate' : 'parallel';
}
export const dynamicViewBranchCollection = (services) => {
    return tryOrLog((node, accept) => {
        ...
        if (totalPaths === 1) {
            accept('warning', `${kind === 'parallel' ? 'Parallel' : 'Alternate'} block with only one path has no branching value. Consider removing the ${kind} wrapper.`, {
                node,
        ...
        if (kind === 'parallel') {
            for (const path of node.paths) {
                ...
                if (path.steps.length === 1 && ast.isDynamicViewBranchCollection(path.steps[0])) {
                    ...
                    if (nestedKind === 'parallel') {
                        accept('error', 'Nested parallel inside parallel with no other steps is not allowed. Parallel blocks are associative - flatten inner parallel paths into the parent parallel.', {
                            node: path.steps[0],
            ...
            for (const step of node.steps) {
                if (ast.isDynamicViewBranchCollection(step)) {
                    ...
                    if (nestedKind === 'parallel') {
                        accept('warning', 'Anonymous nested parallel will create a separate path. Consider using named paths for clarity.', {
                            node: step,
                            code: 'LIKEC4-NESTED-PARALLEL',
                        });
                    }
            ...
            for (const path of node.paths) {
                if (path.steps.length === 1 && ast.isDynamicViewBranchCollection(path.steps[0])) {
                    ...
                    if (nestedKind === 'alternate') {
                        accept('hint', 'Nested alternate inside alternate with no other steps can be flattened. Alternate blocks are associative - consider using sibling paths instead.', {
                            node: path.steps[0],
